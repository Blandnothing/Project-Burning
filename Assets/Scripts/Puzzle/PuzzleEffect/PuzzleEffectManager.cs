...
public class PuzzleEffectManager : MonoBehaviour
{
    ...
    [SerializeField, Header("≤‚ ‘”√")] List<PuzzleEffect> testEffects = new List<PuzzleEffect>();
    static bool isOne;
    private void Awake()
    {
        if (isOne)
    ...
    public void RemovePuzzle(PuzzleEffect effect,KeyCode key)
    {
        if (effect is Buff)
        {
            if (buffs.ContainsKey(key) && buffs[key].Contains(effect as Buff))
            {
                buffs[key].Remove(effect as Buff);
            }
            else
            {
                Debug.LogError("Puzzle  Remove Error");
            }
        }
        else if (effect is Skill)
        {
            if (skills.ContainsKey(key) && skills[key]==effect)
            {
                skills.Remove(key);
            }
            else
